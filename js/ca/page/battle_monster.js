...
tools['Page'].runtime['battle_monster.php'] = function() {
	...
	if($('form:has(div.imgButton > input[alt="Ask for help"]):first').length == 1) {
		$('div > img[src*="siege"]:last').parent().append('<a href="http://apps.facebook.com/castle_age/battle_monster.php?' + $('form:has(div.imgButton > input[alt="Ask for help"]):first').serialize() + '&action=doObjective"><img id="cageSummonCTA" src="http://image4.castleagegame.com/graphics/mp_button_summon.gif"></a>').click(function() {
