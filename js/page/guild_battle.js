...
tools['Page'].cache['guild_battle.php'] = function() {
	...
	// resize top image
	$('#guild_battle_banner_section').css('height', 190).find('div').eq(5).css(
			'marginTop', 0);
	$('div:contains("The Battle Between"):last').parent().css('marginTop', 20);
	$('input[src*="collect_reward_button2.jpg"]').parents('div:eq(2)').css(
			'marginTop', 0);
	...
	var _gate = /\d/.exec($('#enemy_guild_battle_section_battle_list').attr(
			'class'));
	$('#results_main_wrapper form').append(
			'<input type="hidden" name="sel_pos" value="' + _gate + '">');
