...
tools['Heal'].start[com.port.castleAge] = function() {
	...
	- parseInt($('#health_current_value').text(), 10) > 0) {
		if (CastleAge.bqh !== null) {
			$('keep.php?action=heal_avatar&do=heal wounds&bqh=' + CastleAge.bqh
			+ '&signed_request=' + CastleAge.signed_request, function() {
				tools['Heal'].addFunction( function() {
					clearTimeout(timedStats['health']);
					stat_increase_ticker(0, $('#health_current_value')
					.next().text(), $('#health_current_value')
					.next().text(), 0, 0, 'health', false);
				}, null, true, true);
			...
			_data) {
				CastleAge.bqh = $('input[name="bqh"]:first', _data).attr(
				'value');
